...
*/
class taza(var color: String = "blanco", var capacidad: Int = 50, var cantidad: Int = 30){
    enum class colores{
        blanco,
        negro,
        gris,
...
class cafetera(var ubicacion:String){
    ...
    var capacidad_actual:Int= 0
    constructor(ubicacion:String, capacidad:Int ) : this(ubicacion){
        this.capacidad_actual=capacidad
    }
    constructor(ubicacion: String, capacidad: Int, capacidad_actual:Int) : this(ubicacion){

    }

    fun llenar(){
        this.capacidad_actual=this.capacidad
    }
    fun servirTaza() {
    ...
    fun agregarCafe(total:Int){
        if ((capacidad_actual + total) < capacidad){
            capacidad_actual += total
        }


    }
    override fun toString(): String {
        return (" la cafetera esta en ${ubicacion} con el liquido de ${capacidad_actual} y la capacidad de ${capacidad}")
    }
}
...
fun main() {
    ...
    //TODO: Crear 3 cafeteras en la Sala, Cocina y Oficina
        ...
        var cafetera2 = cafetera("cocina", 1000)
        var cafetera3 = cafetera("oficina", 1000, 342)
    //TODO: Crear    una lista de 20 tazas con capacidades aleatorias
    var r = 0
    var j = 0
    var tazas = mutableListOf<taza>()
    for(i in 1..20){ r = Random.nextInt(5);r.toString(r); j += i;j = taza(r,23,23); tazas.add(j)}
